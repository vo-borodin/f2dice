...
import com.voborodin.f2dice.types.IBTController
...
class BTController @Inject constructor(
    ...
    init {
        updatePairedDevices()
        context.registerReceiver(btStateReceiver, IntentFilter().apply {
    ...
    override fun stopDiscovery() {
        if (hasPermission(Manifest.permission.BLUETOOTH_SCAN)) btAdapter?.cancelDiscovery()
    }
    ...
    override fun startBTServer(): Flow<ConnectionResult> = flow {
        ...
        while (shouldLoop) {
            ...
            currentClientSocket?.let {
                ...
                emitAll(service.listenForIncomingMessage().map { msg ->
        ...
        currentClientSocket?.let { socket ->
            try {
                ...
                BTDataTransferService(socket = socket).also {
                    ...
                    emitAll(it.listenForIncomingMessage().map { msg ->
                        ConnectionResult.TransferSucceeded(msg)
                    })
                }
            } catch (e: IOException) {
    ...
    override suspend fun trySendMsg(msg: String): BTMsg? {
        if (!hasPermission(Manifest.permission.BLUETOOTH_CONNECT) || dataTransferService == null) return null

        val btMsg = BTMsg(
            msg = msg, senderName = btAdapter?.name ?: "Unknown name", isFromLocalUser = true
        )
    ...
    private fun updatePairedDevices() {
        if (hasPermission(Manifest.permission.BLUETOOTH_CONNECT)) btAdapter?.bondedDevices?.map { it.toBTDevice() }
